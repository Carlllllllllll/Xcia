  ...
  run: async (client, interaction) => {
    ...
    try {
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription('**Click Current Song To Loop A Singular Song Or Click Queue To Loop The Entire Queue**')
     
      interaction?.reply({ embeds: [embed], components: [button], fetchReply: true }).then(async Message => {

        const filter = i => i.user.id === interaction.user.id
        let col = await Message.createMessageComponentCollector({ filter, time: 100000000 });
